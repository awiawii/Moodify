...
function getJournals(req, res){
    models.Journal.findAll({where: {uid:req.params.id}}).then(result =>{
...
function getTodayJournal(req, res){
    const currentDate = new Date().toISOString().split('T')[0];
    models.Journal.findAll({where: {uid:req.params.id, createdAt:{[Op.between]: [`${currentDate} 00:00:00`, `${currentDate} 23:59:59`]}}}).then(result =>{
        return res.status(200).json({
            journal:result
        });
    }).catch(error =>{
